/**
 * OgulcanUI DemandForecastProjection — one-line enterprise chart (attributes only).
 */
export class OgulcanDemandForecastProjection extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanDemandForecastProjection.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanDemandForecastProjection.MAX_SERIES ? nums.slice(0, OgulcanDemandForecastProjection.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanDemandForecastProjection.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanDemandForecastProjection.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanDemandForecastProjection.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Demand Forecast Projection';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'DemandForecastProjection', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanDemandForecastProjection.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-demand-forecast-projection', OgulcanDemandForecastProjection);
