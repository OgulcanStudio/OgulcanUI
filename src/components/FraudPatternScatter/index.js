/**
 * OgulcanUI FraudPatternScatter — one-line enterprise chart (attributes only).
 */
export class OgulcanFraudPatternScatter extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanFraudPatternScatter.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanFraudPatternScatter.MAX_SERIES ? nums.slice(0, OgulcanFraudPatternScatter.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanFraudPatternScatter.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanFraudPatternScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanFraudPatternScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Fraud Pattern Scatter';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'FraudPatternScatter', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanFraudPatternScatter.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-fraud-pattern-scatter', OgulcanFraudPatternScatter);
