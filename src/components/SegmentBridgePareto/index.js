/**
 * OgulcanUI SegmentBridgePareto — one-line enterprise chart (attributes only).
 */
export class OgulcanSegmentBridgePareto extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanSegmentBridgePareto.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanSegmentBridgePareto.MAX_SERIES ? nums.slice(0, OgulcanSegmentBridgePareto.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanSegmentBridgePareto.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanSegmentBridgePareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanSegmentBridgePareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Segment Bridge Pareto';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'SegmentBridgePareto', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanSegmentBridgePareto.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-segment-bridge-pareto', OgulcanSegmentBridgePareto);
