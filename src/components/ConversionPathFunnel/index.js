/**
 * OgulcanUI ConversionPathFunnel — one-line enterprise chart (attributes only).
 */
export class OgulcanConversionPathFunnel extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanConversionPathFunnel.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanConversionPathFunnel.MAX_SERIES ? nums.slice(0, OgulcanConversionPathFunnel.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanConversionPathFunnel.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanConversionPathFunnel.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanConversionPathFunnel.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Conversion Path Funnel';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ConversionPathFunnel', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanConversionPathFunnel.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-conversion-path-funnel', OgulcanConversionPathFunnel);
