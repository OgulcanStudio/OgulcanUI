/**
 * OgulcanUI LatencyControlChart — one-line enterprise chart (attributes only).
 */
export class OgulcanLatencyControlChart extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanLatencyControlChart.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanLatencyControlChart.MAX_SERIES ? nums.slice(0, OgulcanLatencyControlChart.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanLatencyControlChart.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanLatencyControlChart.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanLatencyControlChart.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Latency Control Chart';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'LatencyControlChart', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanLatencyControlChart.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-latency-control-chart', OgulcanLatencyControlChart);
