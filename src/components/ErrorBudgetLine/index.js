/**
 * OgulcanUI ErrorBudgetLine — one-line enterprise chart (attributes only).
 */
export class OgulcanErrorBudgetLine extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanErrorBudgetLine.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanErrorBudgetLine.MAX_SERIES ? nums.slice(0, OgulcanErrorBudgetLine.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanErrorBudgetLine.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanErrorBudgetLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanErrorBudgetLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Error Budget Line';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ErrorBudgetLine', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanErrorBudgetLine.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-error-budget-line', OgulcanErrorBudgetLine);
