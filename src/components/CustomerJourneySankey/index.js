/**
 * OgulcanUI CustomerJourneySankey — one-line enterprise chart (attributes only).
 */
export class OgulcanCustomerJourneySankey extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanCustomerJourneySankey.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanCustomerJourneySankey.MAX_SERIES ? nums.slice(0, OgulcanCustomerJourneySankey.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanCustomerJourneySankey.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanCustomerJourneySankey.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanCustomerJourneySankey.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Customer Journey Sankey';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'CustomerJourneySankey', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanCustomerJourneySankey.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-customer-journey-sankey', OgulcanCustomerJourneySankey);
