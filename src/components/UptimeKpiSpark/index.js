/**
 * OgulcanUI UptimeKpiSpark — one-line enterprise chart (attributes only).
 */
export class OgulcanUptimeKpiSpark extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanUptimeKpiSpark.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanUptimeKpiSpark.MAX_SERIES ? nums.slice(0, OgulcanUptimeKpiSpark.MAX_SERIES) : nums;
    } catch {
  ...
  static get observedAttributes() {
    return ["title","value","change","trend","sparkline","color"];
  }
  ...
  connectedCallback() {
    ...
    this.upgradeProperty('change');
    this.upgradeProperty('trend');
    this.upgradeProperty('sparkline');
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'title':
        this._title = OgulcanUptimeKpiSpark.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'value':
        this._value = OgulcanUptimeKpiSpark.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'change':
        this._change = OgulcanUptimeKpiSpark.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'trend':
        this._trend = OgulcanUptimeKpiSpark.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'sparkline':
        this._sparkline = OgulcanUptimeKpiSpark.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanUptimeKpiSpark.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  set title(v) {
    ...
    this._title = typeof v === 'object' ? v : s;
  ...
  set change(v) {
    const s = v == null ? '' : (typeof v === 'object' ? JSON.stringify(v) : String(v));
    this._change = typeof v === 'object' ? v : s;
    this.setAttribute('change', s);
  }
  get trend() { return this._trend; }
  set trend(v) {
    const s = v == null ? '' : (typeof v === 'object' ? JSON.stringify(v) : String(v));
    this._trend = typeof v === 'object' ? v : s;
    this.setAttribute('trend', s);
  }
  get sparkline() { return this._sparkline; }
  set sparkline(v) {
    const s = v == null ? '' : (typeof v === 'object' ? JSON.stringify(v) : String(v));
    this._sparkline = typeof v === 'object' ? v : s;
    this.setAttribute('sparkline', s);
  }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Uptime Kpi Spark';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'UptimeKpiSpark', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanUptimeKpiSpark.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-uptime-kpi-spark', OgulcanUptimeKpiSpark);
