/**
 * OgulcanUI VolatilityBoxplot — one-line enterprise chart (attributes only).
 */
export class OgulcanVolatilityBoxplot extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanVolatilityBoxplot.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanVolatilityBoxplot.MAX_SERIES ? nums.slice(0, OgulcanVolatilityBoxplot.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanVolatilityBoxplot.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanVolatilityBoxplot.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanVolatilityBoxplot.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Volatility Boxplot';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'VolatilityBoxplot', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanVolatilityBoxplot.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-volatility-boxplot', OgulcanVolatilityBoxplot);
