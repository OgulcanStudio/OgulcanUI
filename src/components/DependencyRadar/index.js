/**
 * OgulcanUI DependencyRadar — one-line enterprise chart (attributes only).
 */
export class OgulcanDependencyRadar extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanDependencyRadar.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanDependencyRadar.MAX_SERIES ? nums.slice(0, OgulcanDependencyRadar.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanDependencyRadar.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanDependencyRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanDependencyRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Dependency Radar';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'DependencyRadar', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanDependencyRadar.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-dependency-radar', OgulcanDependencyRadar);
