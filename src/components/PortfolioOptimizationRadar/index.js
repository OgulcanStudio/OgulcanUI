/**
 * OgulcanUI PortfolioOptimizationRadar — one-line enterprise chart (attributes only).
 */
export class OgulcanPortfolioOptimizationRadar extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanPortfolioOptimizationRadar.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanPortfolioOptimizationRadar.MAX_SERIES ? nums.slice(0, OgulcanPortfolioOptimizationRadar.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanPortfolioOptimizationRadar.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanPortfolioOptimizationRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanPortfolioOptimizationRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Portfolio Optimization Radar';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'PortfolioOptimizationRadar', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanPortfolioOptimizationRadar.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-portfolio-optimization-radar', OgulcanPortfolioOptimizationRadar);
