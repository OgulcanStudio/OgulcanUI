/**
 * OgulcanUI ThroughputHistogram — one-line enterprise chart (attributes only).
 */
export class OgulcanThroughputHistogram extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanThroughputHistogram.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanThroughputHistogram.MAX_SERIES ? nums.slice(0, OgulcanThroughputHistogram.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanThroughputHistogram.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanThroughputHistogram.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanThroughputHistogram.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Throughput Histogram';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ThroughputHistogram', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanThroughputHistogram.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-throughput-histogram', OgulcanThroughputHistogram);
