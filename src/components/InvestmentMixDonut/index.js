/**
 * OgulcanUI InvestmentMixDonut — one-line enterprise chart (attributes only).
 */
export class OgulcanInvestmentMixDonut extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanInvestmentMixDonut.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanInvestmentMixDonut.MAX_SERIES ? nums.slice(0, OgulcanInvestmentMixDonut.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanInvestmentMixDonut.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanInvestmentMixDonut.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanInvestmentMixDonut.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Investment Mix Donut';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'InvestmentMixDonut', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanInvestmentMixDonut.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-investment-mix-donut', OgulcanInvestmentMixDonut);
