/**
 * OgulcanUI GrowthCurveArea — one-line enterprise chart (attributes only).
 */
export class OgulcanGrowthCurveArea extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanGrowthCurveArea.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanGrowthCurveArea.MAX_SERIES ? nums.slice(0, OgulcanGrowthCurveArea.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanGrowthCurveArea.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanGrowthCurveArea.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanGrowthCurveArea.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Growth Curve Area';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'GrowthCurveArea', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanGrowthCurveArea.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-growth-curve-area', OgulcanGrowthCurveArea);
