/**
 * OgulcanUI ControlCoverageBullet — one-line enterprise chart (attributes only).
 */
export class OgulcanControlCoverageBullet extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanControlCoverageBullet.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanControlCoverageBullet.MAX_SERIES ? nums.slice(0, OgulcanControlCoverageBullet.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanControlCoverageBullet.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanControlCoverageBullet.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanControlCoverageBullet.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Control Coverage Bullet';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ControlCoverageBullet', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanControlCoverageBullet.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-control-coverage-bullet', OgulcanControlCoverageBullet);
