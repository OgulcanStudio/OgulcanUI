/**
 * OgulcanUI BudgetTradeoffWaterfall — one-line enterprise chart (attributes only).
 */
export class OgulcanBudgetTradeoffWaterfall extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanBudgetTradeoffWaterfall.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanBudgetTradeoffWaterfall.MAX_SERIES ? nums.slice(0, OgulcanBudgetTradeoffWaterfall.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanBudgetTradeoffWaterfall.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Budget Tradeoff Waterfall';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'BudgetTradeoffWaterfall', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanBudgetTradeoffWaterfall.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-budget-tradeoff-waterfall', OgulcanBudgetTradeoffWaterfall);
