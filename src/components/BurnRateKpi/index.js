/**
 * OgulcanUI BurnRateKpi — one-line enterprise chart (attributes only).
 */
export class OgulcanBurnRateKpi extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanBurnRateKpi.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanBurnRateKpi.MAX_SERIES ? nums.slice(0, OgulcanBurnRateKpi.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'title':
        this._title = OgulcanBurnRateKpi.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'value':
        this._value = OgulcanBurnRateKpi.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'change':
        this._change = OgulcanBurnRateKpi.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'trend':
        this._trend = OgulcanBurnRateKpi.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'sparkline':
        this._sparkline = OgulcanBurnRateKpi.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanBurnRateKpi.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Burn Rate Kpi';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'BurnRateKpi', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanBurnRateKpi.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-burn-rate-kpi', OgulcanBurnRateKpi);
