/**
 * OgulcanUI AllocationWaterfall — one-line enterprise chart (attributes only).
 */
export class OgulcanAllocationWaterfall extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanAllocationWaterfall.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanAllocationWaterfall.MAX_SERIES ? nums.slice(0, OgulcanAllocationWaterfall.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanAllocationWaterfall.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanAllocationWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanAllocationWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Allocation Waterfall';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'AllocationWaterfall', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanAllocationWaterfall.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-allocation-waterfall', OgulcanAllocationWaterfall);
