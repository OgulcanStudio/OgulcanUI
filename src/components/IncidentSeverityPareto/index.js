/**
 * OgulcanUI IncidentSeverityPareto — one-line enterprise chart (attributes only).
 */
export class OgulcanIncidentSeverityPareto extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanIncidentSeverityPareto.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanIncidentSeverityPareto.MAX_SERIES ? nums.slice(0, OgulcanIncidentSeverityPareto.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanIncidentSeverityPareto.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanIncidentSeverityPareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanIncidentSeverityPareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Incident Severity Pareto';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'IncidentSeverityPareto', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanIncidentSeverityPareto.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-incident-severity-pareto', OgulcanIncidentSeverityPareto);
