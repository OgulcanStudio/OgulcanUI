/**
 * OgulcanUI StrategyFunnel — one-line enterprise chart (attributes only).
 */
export class OgulcanStrategyFunnel extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanStrategyFunnel.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanStrategyFunnel.MAX_SERIES ? nums.slice(0, OgulcanStrategyFunnel.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanStrategyFunnel.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanStrategyFunnel.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanStrategyFunnel.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Strategy Funnel';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'StrategyFunnel', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanStrategyFunnel.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-strategy-funnel', OgulcanStrategyFunnel);
