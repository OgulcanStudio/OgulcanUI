/**
 * OgulcanUI ServiceHealthMatrix — one-line enterprise chart (attributes only).
 */
export class OgulcanServiceHealthMatrix extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanServiceHealthMatrix.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanServiceHealthMatrix.MAX_SERIES ? nums.slice(0, OgulcanServiceHealthMatrix.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanServiceHealthMatrix.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanServiceHealthMatrix.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanServiceHealthMatrix.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Service Health Matrix';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ServiceHealthMatrix', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanServiceHealthMatrix.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-service-health-matrix', OgulcanServiceHealthMatrix);
