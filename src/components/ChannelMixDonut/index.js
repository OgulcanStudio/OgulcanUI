/**
 * OgulcanUI ChannelMixDonut — one-line enterprise chart (attributes only).
 */
export class OgulcanChannelMixDonut extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanChannelMixDonut.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanChannelMixDonut.MAX_SERIES ? nums.slice(0, OgulcanChannelMixDonut.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanChannelMixDonut.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanChannelMixDonut.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanChannelMixDonut.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Channel Mix Donut';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ChannelMixDonut', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanChannelMixDonut.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-channel-mix-donut', OgulcanChannelMixDonut);
