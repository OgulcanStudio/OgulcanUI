/**
 * OgulcanUI ScenarioSensitivityMultiLine — one-line enterprise chart (attributes only).
 */
export class OgulcanScenarioSensitivityMultiLine extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanScenarioSensitivityMultiLine.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanScenarioSensitivityMultiLine.MAX_SERIES ? nums.slice(0, OgulcanScenarioSensitivityMultiLine.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanScenarioSensitivityMultiLine.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Scenario Sensitivity Multi Line';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ScenarioSensitivityMultiLine', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanScenarioSensitivityMultiLine.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-scenario-sensitivity-multi-line', OgulcanScenarioSensitivityMultiLine);
