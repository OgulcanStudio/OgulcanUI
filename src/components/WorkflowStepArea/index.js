/**
 * OgulcanUI WorkflowStepArea — one-line enterprise chart (attributes only).
 */
export class OgulcanWorkflowStepArea extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanWorkflowStepArea.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanWorkflowStepArea.MAX_SERIES ? nums.slice(0, OgulcanWorkflowStepArea.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanWorkflowStepArea.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanWorkflowStepArea.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanWorkflowStepArea.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Workflow Step Area';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'WorkflowStepArea', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanWorkflowStepArea.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-workflow-step-area', OgulcanWorkflowStepArea);
