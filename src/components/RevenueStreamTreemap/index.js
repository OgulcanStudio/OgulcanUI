/**
 * OgulcanUI RevenueStreamTreemap — one-line enterprise chart (attributes only).
 */
export class OgulcanRevenueStreamTreemap extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanRevenueStreamTreemap.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanRevenueStreamTreemap.MAX_SERIES ? nums.slice(0, OgulcanRevenueStreamTreemap.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanRevenueStreamTreemap.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanRevenueStreamTreemap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanRevenueStreamTreemap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Revenue Stream Treemap';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'RevenueStreamTreemap', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanRevenueStreamTreemap.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-revenue-stream-treemap', OgulcanRevenueStreamTreemap);
