/**
 * OgulcanUI ThresholdRing — one-line enterprise chart (attributes only).
 */
export class OgulcanThresholdRing extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanThresholdRing.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanThresholdRing.MAX_SERIES ? nums.slice(0, OgulcanThresholdRing.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'value':
        this._value = OgulcanThresholdRing.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'min':
        this._min = OgulcanThresholdRing.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'max':
        this._max = OgulcanThresholdRing.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanThresholdRing.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanThresholdRing.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Threshold Ring';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ThresholdRing', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanThresholdRing.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-threshold-ring', OgulcanThresholdRing);
