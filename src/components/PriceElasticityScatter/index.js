/**
 * OgulcanUI PriceElasticityScatter — one-line enterprise chart (attributes only).
 */
export class OgulcanPriceElasticityScatter extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanPriceElasticityScatter.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanPriceElasticityScatter.MAX_SERIES ? nums.slice(0, OgulcanPriceElasticityScatter.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanPriceElasticityScatter.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanPriceElasticityScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanPriceElasticityScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Price Elasticity Scatter';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'PriceElasticityScatter', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanPriceElasticityScatter.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-price-elasticity-scatter', OgulcanPriceElasticityScatter);
