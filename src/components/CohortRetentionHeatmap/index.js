/**
 * OgulcanUI CohortRetentionHeatmap — one-line enterprise chart (attributes only).
 */
export class OgulcanCohortRetentionHeatmap extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanCohortRetentionHeatmap.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanCohortRetentionHeatmap.MAX_SERIES ? nums.slice(0, OgulcanCohortRetentionHeatmap.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanCohortRetentionHeatmap.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanCohortRetentionHeatmap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanCohortRetentionHeatmap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Cohort Retention Heatmap';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'CohortRetentionHeatmap', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanCohortRetentionHeatmap.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-cohort-retention-heatmap', OgulcanCohortRetentionHeatmap);
