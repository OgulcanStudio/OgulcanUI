/**
 * OgulcanUI PolicyExceptionHeatmap — one-line enterprise chart (attributes only).
 */
export class OgulcanPolicyExceptionHeatmap extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanPolicyExceptionHeatmap.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanPolicyExceptionHeatmap.MAX_SERIES ? nums.slice(0, OgulcanPolicyExceptionHeatmap.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanPolicyExceptionHeatmap.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanPolicyExceptionHeatmap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanPolicyExceptionHeatmap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Policy Exception Heatmap';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'PolicyExceptionHeatmap', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanPolicyExceptionHeatmap.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-policy-exception-heatmap', OgulcanPolicyExceptionHeatmap);
