/**
 * OgulcanUI ForecastConfidenceBoxplot — one-line enterprise chart (attributes only).
 */
export class OgulcanForecastConfidenceBoxplot extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanForecastConfidenceBoxplot.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanForecastConfidenceBoxplot.MAX_SERIES ? nums.slice(0, OgulcanForecastConfidenceBoxplot.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanForecastConfidenceBoxplot.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanForecastConfidenceBoxplot.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanForecastConfidenceBoxplot.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Forecast Confidence Boxplot';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ForecastConfidenceBoxplot', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanForecastConfidenceBoxplot.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-forecast-confidence-boxplot', OgulcanForecastConfidenceBoxplot);
