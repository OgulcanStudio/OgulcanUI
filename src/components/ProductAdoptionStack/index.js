/**
 * OgulcanUI ProductAdoptionStack — one-line enterprise chart (attributes only).
 */
export class OgulcanProductAdoptionStack extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanProductAdoptionStack.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanProductAdoptionStack.MAX_SERIES ? nums.slice(0, OgulcanProductAdoptionStack.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanProductAdoptionStack.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanProductAdoptionStack.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanProductAdoptionStack.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Product Adoption Stack';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ProductAdoptionStack', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanProductAdoptionStack.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-product-adoption-stack', OgulcanProductAdoptionStack);
