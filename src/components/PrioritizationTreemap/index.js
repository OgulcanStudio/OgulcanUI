/**
 * OgulcanUI PrioritizationTreemap — one-line enterprise chart (attributes only).
 */
export class OgulcanPrioritizationTreemap extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanPrioritizationTreemap.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanPrioritizationTreemap.MAX_SERIES ? nums.slice(0, OgulcanPrioritizationTreemap.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanPrioritizationTreemap.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanPrioritizationTreemap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanPrioritizationTreemap.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Prioritization Treemap';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'PrioritizationTreemap', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanPrioritizationTreemap.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-prioritization-treemap', OgulcanPrioritizationTreemap);
