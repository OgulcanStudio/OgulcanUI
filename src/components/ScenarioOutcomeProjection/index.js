/**
 * OgulcanUI ScenarioOutcomeProjection — one-line enterprise chart (attributes only).
 */
export class OgulcanScenarioOutcomeProjection extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanScenarioOutcomeProjection.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanScenarioOutcomeProjection.MAX_SERIES ? nums.slice(0, OgulcanScenarioOutcomeProjection.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanScenarioOutcomeProjection.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanScenarioOutcomeProjection.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanScenarioOutcomeProjection.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Scenario Outcome Projection';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ScenarioOutcomeProjection', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanScenarioOutcomeProjection.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-scenario-outcome-projection', OgulcanScenarioOutcomeProjection);
