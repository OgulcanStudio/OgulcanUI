/**
 * OgulcanUI ExposureLimitBars — one-line enterprise chart (attributes only).
 */
export class OgulcanExposureLimitBars extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanExposureLimitBars.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanExposureLimitBars.MAX_SERIES ? nums.slice(0, OgulcanExposureLimitBars.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanExposureLimitBars.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanExposureLimitBars.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanExposureLimitBars.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Exposure Limit Bars';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ExposureLimitBars', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanExposureLimitBars.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-exposure-limit-bars', OgulcanExposureLimitBars);
