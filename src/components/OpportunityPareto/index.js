/**
 * OgulcanUI OpportunityPareto — one-line enterprise chart (attributes only).
 */
export class OgulcanOpportunityPareto extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanOpportunityPareto.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanOpportunityPareto.MAX_SERIES ? nums.slice(0, OgulcanOpportunityPareto.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanOpportunityPareto.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanOpportunityPareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanOpportunityPareto.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Opportunity Pareto';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'OpportunityPareto', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanOpportunityPareto.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-opportunity-pareto', OgulcanOpportunityPareto);
