/**
 * OgulcanUI RiskAppetiteGauge — one-line enterprise chart (attributes only).
 */
export class OgulcanRiskAppetiteGauge extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanRiskAppetiteGauge.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanRiskAppetiteGauge.MAX_SERIES ? nums.slice(0, OgulcanRiskAppetiteGauge.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'value':
        this._value = OgulcanRiskAppetiteGauge.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'min':
        this._min = OgulcanRiskAppetiteGauge.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'max':
        this._max = OgulcanRiskAppetiteGauge.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanRiskAppetiteGauge.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanRiskAppetiteGauge.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Risk Appetite Gauge';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'RiskAppetiteGauge', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanRiskAppetiteGauge.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-risk-appetite-gauge', OgulcanRiskAppetiteGauge);
