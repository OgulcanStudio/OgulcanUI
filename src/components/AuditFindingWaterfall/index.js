/**
 * OgulcanUI AuditFindingWaterfall — one-line enterprise chart (attributes only).
 */
export class OgulcanAuditFindingWaterfall extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanAuditFindingWaterfall.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanAuditFindingWaterfall.MAX_SERIES ? nums.slice(0, OgulcanAuditFindingWaterfall.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanAuditFindingWaterfall.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanAuditFindingWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanAuditFindingWaterfall.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Audit Finding Waterfall';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'AuditFindingWaterfall', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanAuditFindingWaterfall.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-audit-finding-waterfall', OgulcanAuditFindingWaterfall);
