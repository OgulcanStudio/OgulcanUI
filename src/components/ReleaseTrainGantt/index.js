/**
 * OgulcanUI ReleaseTrainGantt — one-line enterprise chart (attributes only).
 */
export class OgulcanReleaseTrainGantt extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanReleaseTrainGantt.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanReleaseTrainGantt.MAX_SERIES ? nums.slice(0, OgulcanReleaseTrainGantt.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanReleaseTrainGantt.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanReleaseTrainGantt.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanReleaseTrainGantt.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Release Train Gantt';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ReleaseTrainGantt', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanReleaseTrainGantt.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-release-train-gantt', OgulcanReleaseTrainGantt);
