/**
 * OgulcanUI CapacityStackedTrend — one-line enterprise chart (attributes only).
 */
export class OgulcanCapacityStackedTrend extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanCapacityStackedTrend.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanCapacityStackedTrend.MAX_SERIES ? nums.slice(0, OgulcanCapacityStackedTrend.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanCapacityStackedTrend.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanCapacityStackedTrend.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanCapacityStackedTrend.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Capacity Stacked Trend';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'CapacityStackedTrend', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanCapacityStackedTrend.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-capacity-stacked-trend', OgulcanCapacityStackedTrend);
