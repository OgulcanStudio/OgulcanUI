/**
 * OgulcanUI SensorDriftScatter — one-line enterprise chart (attributes only).
 */
export class OgulcanSensorDriftScatter extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanSensorDriftScatter.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanSensorDriftScatter.MAX_SERIES ? nums.slice(0, OgulcanSensorDriftScatter.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanSensorDriftScatter.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanSensorDriftScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanSensorDriftScatter.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Sensor Drift Scatter';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'SensorDriftScatter', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanSensorDriftScatter.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-sensor-drift-scatter', OgulcanSensorDriftScatter);
