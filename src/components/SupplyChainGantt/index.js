/**
 * OgulcanUI SupplyChainGantt — one-line enterprise chart (attributes only).
 */
export class OgulcanSupplyChainGantt extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanSupplyChainGantt.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanSupplyChainGantt.MAX_SERIES ? nums.slice(0, OgulcanSupplyChainGantt.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanSupplyChainGantt.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanSupplyChainGantt.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanSupplyChainGantt.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Supply Chain Gantt';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'SupplyChainGantt', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanSupplyChainGantt.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-supply-chain-gantt', OgulcanSupplyChainGantt);
