/**
 * OgulcanUI MarketPulseLine — one-line enterprise chart (attributes only).
 */
export class OgulcanMarketPulseLine extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanMarketPulseLine.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanMarketPulseLine.MAX_SERIES ? nums.slice(0, OgulcanMarketPulseLine.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanMarketPulseLine.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanMarketPulseLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanMarketPulseLine.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Market Pulse Line';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'MarketPulseLine', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanMarketPulseLine.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-market-pulse-line', OgulcanMarketPulseLine);
