/**
 * OgulcanUI AccessPostureRadar — one-line enterprise chart (attributes only).
 */
export class OgulcanAccessPostureRadar extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanAccessPostureRadar.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanAccessPostureRadar.MAX_SERIES ? nums.slice(0, OgulcanAccessPostureRadar.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanAccessPostureRadar.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanAccessPostureRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanAccessPostureRadar.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Access Posture Radar';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'AccessPostureRadar', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanAccessPostureRadar.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-access-posture-radar', OgulcanAccessPostureRadar);
