/**
 * OgulcanUI WorkforceUtilizationHBars — one-line enterprise chart (attributes only).
 */
export class OgulcanWorkforceUtilizationHBars extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanWorkforceUtilizationHBars.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanWorkforceUtilizationHBars.MAX_SERIES ? nums.slice(0, OgulcanWorkforceUtilizationHBars.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanWorkforceUtilizationHBars.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanWorkforceUtilizationHBars.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanWorkforceUtilizationHBars.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Workforce Utilization HBars';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'WorkforceUtilizationHBars', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanWorkforceUtilizationHBars.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-workforce-utilization-hbars', OgulcanWorkforceUtilizationHBars);
