/**
 * OgulcanUI ComplianceObligationMatrix — one-line enterprise chart (attributes only).
 */
export class OgulcanComplianceObligationMatrix extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanComplianceObligationMatrix.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanComplianceObligationMatrix.MAX_SERIES ? nums.slice(0, OgulcanComplianceObligationMatrix.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanComplianceObligationMatrix.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanComplianceObligationMatrix.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanComplianceObligationMatrix.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Compliance Obligation Matrix';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'ComplianceObligationMatrix', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanComplianceObligationMatrix.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-compliance-obligation-matrix', OgulcanComplianceObligationMatrix);
