/**
 * OgulcanUI AnomalyBandControl — one-line enterprise chart (attributes only).
 */
export class OgulcanAnomalyBandControl extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanAnomalyBandControl.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanAnomalyBandControl.MAX_SERIES ? nums.slice(0, OgulcanAnomalyBandControl.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanAnomalyBandControl.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanAnomalyBandControl.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanAnomalyBandControl.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Anomaly Band Control';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'AnomalyBandControl', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanAnomalyBandControl.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-anomaly-band-control', OgulcanAnomalyBandControl);
