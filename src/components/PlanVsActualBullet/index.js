/**
 * OgulcanUI PlanVsActualBullet — one-line enterprise chart (attributes only).
 */
export class OgulcanPlanVsActualBullet extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanPlanVsActualBullet.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanPlanVsActualBullet.MAX_SERIES ? nums.slice(0, OgulcanPlanVsActualBullet.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanPlanVsActualBullet.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanPlanVsActualBullet.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanPlanVsActualBullet.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Plan Vs Actual Bullet';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'PlanVsActualBullet', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanPlanVsActualBullet.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-plan-vs-actual-bullet', OgulcanPlanVsActualBullet);
