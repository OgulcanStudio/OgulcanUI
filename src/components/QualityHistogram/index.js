/**
 * OgulcanUI QualityHistogram — one-line enterprise chart (attributes only).
 */
export class OgulcanQualityHistogram extends HTMLElement {
  ...
  static parseSeriesAttr(raw) {
    if (!raw || String(raw).length > OgulcanQualityHistogram.MAX_JSON_ATTR) return [];
    try {
      const j = JSON.parse(raw);
      if (!Array.isArray(j)) return [];
      const nums = j.map((p) => (typeof p === 'number' ? p : Number(p && (p.value ?? p.y)))).filter((n) => Number.isFinite(n));
      return nums.length > OgulcanQualityHistogram.MAX_SERIES ? nums.slice(0, OgulcanQualityHistogram.MAX_SERIES) : nums;
    } catch {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'data':
        this._data = OgulcanQualityHistogram.parseSeriesAttr(newValue);
        this._paintSig = '';
        break;
      case 'label':
        this._label = OgulcanQualityHistogram.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
      case 'color':
        this._color = OgulcanQualityHistogram.sanitizeAttr(newValue);
        this._paintSig = '';
        break;
    }
  ...
  chartA11ySummary(ctx) {
    const label = ctx.label || ctx.title || 'Quality Histogram';
  ...
  collectPaintContext() {
    const el = this;
    const ctx = { component: 'QualityHistogram', svg: this.svg, tooltip: this.tooltip, shadowRoot: this.shadowRoot };
    ctx.label = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('label') || el.getAttribute('title') || '');
    ctx.color = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('color') || el.getAttribute('theme-color') || '', 64);
    ...
    ctx.value = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('value'), 64);
    ctx.title = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('title'));
    ctx.change = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('change'), 128);
    ctx.trend = OgulcanQualityHistogram.sanitizeAttr(el.getAttribute('trend'), 32);
...
customElements.define('ogulcan-quality-histogram', OgulcanQualityHistogram);
